...
- **Application**: `placeOrder`, `getOrderSummary`, `dispatchOutbox`, `pollOutbox`, authorization policy
- **Ports**: catalog / repository / read model / payment / outbox / integration event publisher / integration event subscriber / unit of work / observability / audit log
- **Adapters**:
  - in-memory
  - SQLite
  - PostgreSQL
  ...
  - integration event publisher (console / broker-like)
...
### レイヤー対応
- **Domain**: `Order`, `Money`
- **Application / Use Case**: `placeOrder`, `getOrderSummary`, `dispatchOutbox`, `pollOutbox`
- **Ports**: `ProductCatalogPort`, `OrderRepositoryPort`, `OrderReadModelPort`, `PaymentGatewayPort`, `OutboxPort`, `IntegrationEventPublisherPort`, `IntegrationEventSubscriberPort`, `UnitOfWorkPort`, `ObservabilityPort`, `AuditLogPort`
- **Adapters**: in-memory / SQLite / PostgreSQL / outbox / read model / payment / console / broker-like / subscribers / HTTP / CLI / batch
...
本物の常駐 worker ではありませんが、**request の外で delivery を回す責務**を説明するには十分です。
...
さらに深めるなら、次は **Sprint 7〜12 を第1区切り** として進めます。
詳細は `docs/19-sprint7-12-roadmap.md` を参照してください。

1. Sprint 7: dedicated worker / cron / queue consumer
2. Sprint 8: broker-like publisher を real broker adapter へ進める
3. Sprint 9: subscriber ごとの failure policy / replay / re-drive
4. Sprint 10: trace / metrics / structured logging を強化する
5. Sprint 11: payment workflow を saga / compensation まで広げる
6. Sprint 12: bounded context / ACL / 「フル採用か部分採用か」の判断へつなぐ
