...
- **Application**: `placeOrder`, `runOrderCheckoutSaga`, `getOrderSummary`, `dispatchOutbox`, `pollOutbox`, `replaySubscriberFailures`, authorization policy
- **Ports**: catalog / repository / read model / payment / fulfillment / outbox / integration event publisher / integration event subscriber / subscriber failure store / subscriber failure policy / unit of work / observability / audit log
- **Adapters**:
  - in-memory
  - SQLite
  - PostgreSQL
...
- **Saga / compensation / workflow orchestration の入口**
- **Bounded context / ACL / strategic DDD の入口**
...
- `docs/29-sprint12-bounded-context-and-acl.md`
...
### レイヤー対応
- **Domain**: `Order`, `Money`
- **Application / Use Case**: `placeOrder`, `runOrderCheckoutSaga`, `getOrderSummary`, `dispatchOutbox`, `pollOutbox`, `replaySubscriberFailures`
- **Ports**: `ProductCatalogPort`, `OrderRepositoryPort`, `OrderReadModelPort`, `PaymentGatewayPort`, `FulfillmentPort`, `OutboxPort`, `IntegrationEventPublisherPort`, `IntegrationEventSubscriberPort`, `SubscriberDeliveryFailureStorePort`, `SubscriberFailurePolicyPort`, `UnitOfWorkPort`, `ObservabilityPort`, `AuditLogPort`
- **Adapters**: in-memory / SQLite / PostgreSQL / outbox / read model / payment / fulfillment / warehouse ACL / console / broker-like / nats / subscribers / worker / HTTP / CLI / batch
...
### Fulfillment service の切り替え
```bash
FULFILLMENT_SERVICE=fake npm run dev -- saga
FULFILLMENT_SERVICE=failing npm run dev -- saga
FULFILLMENT_SERVICE=warehouse-acl npm run dev -- saga
```
...
compensation path を見たい場合は:

```bash
FULFILLMENT_SERVICE=failing npm run dev -- saga
```

ACL adapter path を見たい場合は:

```bash
FULFILLMENT_SERVICE=warehouse-acl npm run dev -- saga
```

`saga` モードは、Sprint 11 で追加した **payment workflow / saga / compensation** の最小デモです。
Sprint 12 ではここに **warehouse context への ACL translation** が加わります。

このデモでは、

- payment を成功させる
- 次に fulfillment step を呼ぶ
- fulfillment が失敗したら refund で compensation する
- `FULFILLMENT_SERVICE=warehouse-acl` では warehouse 側 request / response への翻訳を adapter に閉じる
- compensation 成功 / 失敗を `workflowStatus` と telemetry / audit で確認する

という流れを 1 本で見せます。

ここで見せたいのは、

- local transaction だけでは外部依存の成功を巻き戻せない
- retry と compensation は別の責務
- saga coordinator は use case 間の orchestration として説明できる
- warehouse 側の用語を core に入れず ACL adapter で翻訳できる
...
- local transaction で守れるものと守れないものをどう分けるか
- retry と compensation をなぜ同じ失敗処理として扱わないのか
- saga coordinator を controller や adapter ではなく application に置く意味は何か

### 7. Bounded context / ACL / strategic DDD
- Sales / Payment / Warehouse の文脈差をどう説明するか
- ACL を単なる format 変換ではなく meaning の保護として話せるか
- full adoption / partial adoption / non-adoption をどう判断するか

### 8. Auth と policy placement
...
- retry / replay / compensation をどう使い分けるか
- bounded context / ACL をどこまで採用すべきかをどう判断するか
...
Sprint 12 で、この repo の **第1区切り** はいったん完了です。

さらに深めるなら、次は次のような論点が自然です。

1. workflow state の durable persistence / operator recovery flow
2. consumer group / durable consumer / multi-node worker orchestration
3. schema registry / contract governance / compatibility check
4. context split を本当に別 deployable / 別 service にする判断
