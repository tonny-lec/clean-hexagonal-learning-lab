...
- **Application**: `placeOrder`, `getOrderSummary`, authorization policy
- **Ports**: catalog / repository / payment / outbox / event publisher / unit of work
- **Adapters**:
  - in-memory
  - SQLite
  - PostgreSQL
...
- **Outbox / eventual consistency の入口**
...
- **Ports**: `ProductCatalogPort`, `OrderRepositoryPort`, `PaymentGatewayPort`, `OutboxPort`, `DomainEventPublisherPort`, `UnitOfWorkPort`
- **Adapters**: in-memory / SQLite / PostgreSQL / outbox / console / HTTP / CLI / batch
...
1. Outbox dispatcher / poller を追加する
2. Stripe / Fake / Failure の 3 種類の payment adapter を作る
3. read model を分離して query side を深める
4. integration event と domain event を分ける
5. audit log / observability を足す
